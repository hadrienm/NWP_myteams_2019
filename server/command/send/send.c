...
void send_c(client_t *client, server_data **server)
{
    ctos_send_t sender;
    ...
    if (client->uuid == NULL) {
        ...
        if (found_user(sender.uuid, (*server)->client) == false)
            rfc = set_send_rfc(502, sender.uuid);
        else {
            set_send_s(&client, sender, &(*server)->client);
            save_message(sender, client->uuid, &(*server)->crp_list);
